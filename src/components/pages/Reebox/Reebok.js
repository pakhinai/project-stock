...
export default function Reebox(props) {
  ...
  useEffect(() => {
    dispatch(reeboxAction.getReebox());
  }, []);
      ...
      <div>
        ...
        >
          ...
          <DialogActions>
            ...
            <Button
              onClick={() => {
                dispatch(reeboxAction.removeReebox(selectId));
                handleClose();
                setTimeout(() => {
  ...
  const actions = [
    {
      ...
      onClick: (event, rowData) => {
        props.history.push(`/reebox-edit/${rowData.id}`);
      },
    },
    {
      icon: () => <DeleteOutline />,
  ...
  return (
    <div style={{ width: "100%", height: "100vh", marginTop: 63 }}>
      <MaterialTable
        ...
        columns={columns}
        title="Reebox"
        data={reeboxReducer.reeboxResult ? reeboxReducer.reeboxResult : []}
        components={{
          Toolbar: (props) => (
            <div>
              <MTableToolbar {...props} />
