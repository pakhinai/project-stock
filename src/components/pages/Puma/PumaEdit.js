...
export default function PumaEdit(props) {
  const classes = useStyles();
  const dispatch = useDispatch();
  const pumaReducer = useSelector(({pumaReducer}) => pumaReducer)
  let id = props.match.params.id

  useEffect(() => {   
      dispatch(pumaAction.getPumaById(id))
  }, [])
  ...
  const formik = useFormik({
    ...
    onSubmit: (values) => {
      ...
      dispatch(pumaAction.editPuma(formData, props.match.params.id, props.history));
    },
  });
        ...
        <Card className={classes.root}>
          <CardContent>
            <Typography gutterBottom variant="h5" component="div">
              Edit Puma {props.match.params.id}
            </Typography>
