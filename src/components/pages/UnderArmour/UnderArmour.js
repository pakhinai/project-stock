...
export default function UnderArmour(props) {
  ...
  useEffect(() => {
    dispatch(underArmourAction.getUnderArmour());
  }, []);
      ...
      <div>
        ...
        >
          ...
          <DialogActions>
            ...
            <Button
              onClick={() => {
                dispatch(underArmourAction.removeUnderArmour(selectId));
                handleClose();
                setTimeout(() => {
  ...
  const actions = [
    {
      ...
      onClick: (event, rowData) => {
        props.history.push(`/underarmour-edit/${rowData.id}`);
      },
    },
    {
      icon: () => <DeleteOutline />,
  ...
  return (
    <div style={{ width: "100%", height: "100vh", marginTop: 63 }}>
      <MaterialTable
        ...
        columns={columns}
        title="Under Armour"
        data={underArmourReducer.underArmourResult ? underArmourReducer.underArmourResult : []}
        components={{
          Toolbar: (props) => (
            <div>
              <MTableToolbar {...props} />
